			...
			//if (!undeploy) {
				...
				if (deployRepository) {
...

					{
						...
						bat "C:/MuleSOft/apache-maven-3.5.4-bin/apache-maven-3.5.4/bin/mvn deploy:deploy-file -Durl=${NEXUS_URL}/${NEXUS_REPOSITORY}/ -DrepositoryId=${NEXUS_REPOSITORYID} -DgroupId=${GROUP_ID} -DartifactId='mule-demo' -Dversion=$POM_VERSION -Dpackaging=target/$INSTALL_FILE_NAME-$POM_VERSION.zip -Dfile='target/$INSTALL_FILE_NAME-$POM_VERSION.zip' -DgeneratePom=true"
