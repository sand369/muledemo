			...
			//if (!undeploy) {
				...
				if (!skipMunitTest) {
					...
					stage('MUnit Testing') {
						try{
						def err=''
						bat bat 'C:/MuleSOft/apache-maven-3.5.4-bin/apache-maven-3.5.4/bin/mvn clean test -Dmule.env=local -Dvault.key=capitalfirst@123 -Denv.properties.location=/C:/MuleSOft/conf'
						} 
						catch (err) 
						{
